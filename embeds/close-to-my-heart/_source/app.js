;(function ( global ) {
  ...
  var wantsVideo = (function() {
    ...
    var elements = document.getElementsByClassName("close-to-my-heart__block")[0];

    var elementTop = window.pageYOffset + elements.getBoundingClientRect().top

    var elementTBottom = elementTop - window.innerHeight;

    var video_duration = 2 // In seconds

    var pixels_per_second = window.innerHeight / video_duration

    elements.addEventListener('click',() => {
      window.location.href = "https://www.theguardian.com/lifeandstyle/ng-interactive/2018/may/17/close-to-my-heart-a-guardian-video-series";
    });

    //console.log(elementTop + ' | ' + elementTBottom + ' | ' +  pixels_per_second)
    ...
    var getsVideo = function() {
        setTimeout(function() {
            ...
            if (wantsVideo.canVideo() && window.requestAnimationFrame) {

              var videoCandidates = document.getElementById("videoplayer")
              var video = document.createElement('video');
              video.src = videoCandidates.getAttribute('data-close');
              ...
              var renderLoop = function(){
                requestAnimationFrame( function(){
                  //video.currentTime = (window.pageYOffset/400)*2;
