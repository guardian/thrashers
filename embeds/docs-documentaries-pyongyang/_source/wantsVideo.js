;(function ( global ) {
  ...
  var wantsVideo = (function() {
    ...
    function isIOSRecent() {
      return (navigator.userAgent.indexOf('Safari')>-1 && navigator.userAgent.indexOf('Version/1')>-1);
    }
    ...
    var getsVideo = function() {
      var docName = 'docs-documentaries-pyongyang';

      var checkInterval = setInterval(function() {
        if (document.querySelector('.'+docName+'__wrapper')) {

          if (wantsVideo.canVideo()) {
            var videoCandidates = document.querySelectorAll('[data-wants-'+docName+']');
            for (var i = 0; i < videoCandidates.length; i++) {
              var img = videoCandidates[i];
              var imageWrapper = videoCandidates[i].parentElement;

              var videoSource = img.getAttribute('data-wants-'+docName+'');
              var videoPoster = img.getAttribute('src');

              var videoTag = document.createElement('div');
              videoTag.setAttribute('class', 'video');
              videoTag.innerHTML = '<video playsinline muted="true" poster="'+videoPoster+'" loop="true"><source src="'+videoSource+'" type="video/mp4"></video>';

              imageWrapper.insertBefore(videoTag, img);

              var video = imageWrapper.querySelector('video');
              video.load();
              video.play();
