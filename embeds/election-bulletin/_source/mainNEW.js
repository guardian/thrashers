...
loadJSON("https://interactive.guim.co.uk/docsdata-test/1rCOvGZJZzUxIQ3CqQXypMxAAJXoX4ddm_iEv325T1BY.json", function(data) {
    ...
    function initStories(){
        var storiesString = '';
        var storiesArray = [];
        for(var j = 0; j < data.sheets.bulletin; j++){
	...
	if(showEnd === true) {
		...
		story.style.display = "none"; //css("display", "none");
    ...
    function storyTimerF(){
      var storyTimer = setInterval(function(){
        slide ++;
        document.querySelector('.blankIndicator').classList.remove('active'); //removeClass('active');
        document.querySelector('.indicator-'+slide).classList.add('active'); //addClass('active');
        count = countStart;
        ...
        if(n === null) {
          ...
          button.style.display = "none"; //css("display", "none");
          story.style.display = "none"; //css("display", "none");
		  Showindicator.style.display = "none"; //.removeClass('active');
          c.classList.add('end');
        }
    ...
    //   console.log(getLength() * 8,'total time');
      ...
      button.addEventListener('click', function(){ //.click(function(){
        ...
        secondsTimerF();
        storyTimerF();
        clicker.addEventListener('click', function(){ //.click(function(){
          slide ++;
          clickPercentage();
		  $('.blankIndicator').classList.remove('active'); //.removeClass('active');
		  $('.indicator-'+slide).classList.add('active'); //.addClass('active');
          var oj = document.querySelector('ul.bulletin--onward');
          var a = document.querySelector('li.active');
          var n = document.querySelector('li.active + li.story');
          if(n === null){
      ...
      feedback.addEventListner('click', function(){ //.click(function(){
          feedback.style.display = "none"; //("display", "none");
          notification.style.display = "none"; //.css("display", "block");
      });
      cEnd.addEventListner('click', function(){  //.click(function(){
		count = countStart;
        w = 0;
		secondsTimerF();
        story[0].classList.add('active'); //.addClass('active');
